...
console.log('\n=== 15. Волна не раздувает чужой протокол, минимум остаётся минимумом ===');
{
  ...
  // (б7) Критерий выбора гири есть у каждой программы, и он не врёт про точность:
  //      где сказано «проверяется тестом» — там задан диапазон, который тест меряет.
  for (const [pid, prog] of Object.entries(PROGRAMS)) {
    const r = prog.bellRule;
    ok(!!r && typeof r.текст === 'string' && r.текст.length > 20,
       `${pid}: не записан критерий выбора гири`);
    if (!r) continue;
    if (r.тест) {
      ok(Array.isArray(r.диапазон) && r.диапазон.length === 2 && r.диапазон[0] < r.диапазон[1],
         `${pid}: критерий объявлен проверяемым тестом «${r.тест}», но диапазон не задан`);
      ok(r.источник === true, `${pid}: тестом проверяем только критерии, взятые у автора`);
    } else {
