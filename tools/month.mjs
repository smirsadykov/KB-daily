...
function прогон(pid, профиль, seed = 7) {
  ...
  if (PROGRAMS[pid].noProgression) {
    // автор прогрессии не предусмотрел: приложение о ней и не заговаривает
    if (предложений > 0) баг(pid, 'прогрессия', `${профиль}: программа без прогрессии выдала ${предложений} предложений`);
  } else {
